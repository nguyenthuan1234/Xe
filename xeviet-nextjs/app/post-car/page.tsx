...
import { createCar, updateCar, fetchCarById } from "@/lib/api-cars";
...
interface ImageItem {
  previewUrl: string;
  file?: File; // có nghĩa là ảnh mới, cần upload khi submit
  existingUrl?: string; // ảnh cũ đã có sẵn trên server (chế độ sửa tin)
}

function PostCarPageInner() {
  ...
  // Step 2 — ảnh (dùng chung cho cả tạo mới & sửa tin)
  ...
  // Điền sẵn SĐT theo hồ sơ nếu là tạo tin mới
  useEffect(() => {
    if (!isEditMode && user?.phone) setSellerPhone(user.phone);
  }, [user, isEditMode]);

  // Chế độ SỬA TIN: tải dữ liệu xe cũ và điền sẵn vào form
  useEffect(() => {
    if (!editId) return;
    setLoadingCar(true);
    fetchCarById(editId)
      .then((car) => {
        ...
        const suffix = car.name.startsWith(car.brand)
          ? car.name.slice(car.brand.length).trim()
          : car.name;
      ...
      .catch((err) =>
        setError(
          err instanceof ApiError
            ? err.message
            : "Không tải được thông tin xe cần sửa.",
        ),
      )
  ...
  const handleFilesSelected = (files: FileList | null) => {
    if (!files) return;
    const arr = Array.from(files).slice(0, 20 - imageItems.length);
    arr.forEach((file) => {
      const reader = new FileReader();
      reader.onload = () => {
        setImageItems((prev) => [
          ...prev,
          { previewUrl: reader.result as string, file },
        ]);
      };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const finalImages = imageItems.map((item) =>
        item.existingUrl ? item.existingUrl : uploadedUrls[uploadIdx++],
      );
      ...
      setError(
        err instanceof ApiError
          ? err.message
          : "Lưu tin đăng thất bại. Vui lòng thử lại.",
      );
    } finally {
      setSubmitting(false);
      setUploading(false);
  ...
  if (authLoading || !user || loadingCar) {
    return (
      ...
      <div className="max-w-3xl mx-auto px-4 sm:px-6 pt-24 pb-14">
        <div className="mb-8">
          <h1 className="text-2xl font-black text-slate-900">
            {isEditMode ? "Chỉnh sửa tin đăng" : "Đăng tin bán xe"}
          </h1>
          <p className="text-slate-500 text-sm mt-1">
            {isEditMode
              ? "Sau khi lưu, tin sẽ được đưa về trạng thái chờ duyệt lại."
              : `Hoàn thành ${total} bước — tin đăng sẽ được admin duyệt trước khi hiển thị công khai`}
          </p>
        </div>
        ...
        <div className="flex items-center mb-8">
          {labels.map((l, i) => (
            <div key={l} className="flex items-center flex-1">
              ...
              {i < labels.length - 1 && (
                <div
                  className={`flex-1 h-0.5 mx-2 mt-[-18px] transition-colors ${i + 1 < step ? "bg-emerald-400" : "bg-slate-200"}`}
                />
              )}
            </div>
          ))}
        ...
        <div className="bg-white rounded-2xl border border-slate-100 shadow-sm p-6">
          {step === 1 && (
            <div className="space-y-4">
              ...
              <Field
                label="Tên/phiên bản xe (vd: Camry 2.5Q)"
                placeholder="Nhập tên hoặc phiên bản"
                value={nameSuffix}
            ...
            <div className="space-y-6">
              <div>
                ...
                <p className="text-slate-500 text-xs mb-4">
                  Tải lên tối đa 20 ảnh (JPG, PNG, WEBP). Ảnh đầu tiên làm ảnh
                  đại diện.
                </p>
                    ...
                    >
                      ...
                      <div className="absolute inset-0 bg-black/40 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center">
                        <button
                          ...
                          className="bg-red-500 text-white w-7 h-7 rounded-full flex items-center justify-center shadow-md"
                        >
                          <X size={12} />
                        </button>
                      </div>
          ...
          {step === 3 && (
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-2.5 text-sm text-slate-500 bg-slate-50 rounded-xl px-3.5 py-2.5">
                <User size={14} className="text-blue-500" /> {user.name} (
                {user.email})
              </div>
              <Field
                ...
                <div className="rounded-2xl border border-slate-100 overflow-hidden max-w-xs">
                  ...
                  <div className="p-4">
                    ...
                    <p className="text-xs text-slate-400 mt-1">
                      {year} ·{" "}
                      {km ? `${Number(km).toLocaleString("vi-VN")} km` : "—"} ·{" "}
                      {location || "—"}
                    </p>
                  </div>
                </div>
        ...
        <div className="flex justify-between mt-5">
          ...
          <Btn
            onClick={() => (step < total ? goNext() : handleSubmit())}
            disabled={submitting}
          >
            {submitting ? (
              uploading ? (
